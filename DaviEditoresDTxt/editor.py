...
height = main_application.winfo_screenheight()
width = main_application.winfo_screenwidth()
main_application.geometry(f'{width}x{height}')
main_application.title("Davi editores")


#sugestion parameters
temperature = 0.7 # enquanto mais alto mais criativo
max_tokens = 256 # enquanto maior o numero mais texto vai ser sugerido
...
def exemples():
    webbrowser.open('https://beta.openai.com/examples/')

#btn that has the same size of speak btn
ex_btn = ttk.Button(tool_bar, text="Exemplos", compound="left", command=exemples)
ex_btn.grid(row=0, column=10, padx=5)
...
def Open_sugestion_window(event=None):
    ...
    global max_tokens #enquanto maior o numero mais texto vai ser sugerido
    ...
    t = tk.DoubleVar()
    tp_scale = tk.Scale(window, from_=0.0, to=1.0, orient=tk.HORIZONTAL, sliderlength=60,  length=400, showvalue=1.0, tickinterval=0.0, resolution=0.1, variable=t)
    ...
    #max_tokens
    mt_lb = tk.Label(window, text="Tamanho do texto", font=("Arial", 12), fg="black")
    mt_lb.grid(row=3, column=0)
    
    mp = tk.DoubleVar()
    mp_scale = tk.Scale(window, from_=60, to=1000, orient=tk.HORIZONTAL, sliderlength=60,  length=400, showvalue=1.0, tickinterval=0.0, resolution=0.0, variable=mp)
    mp_scale.grid(row=3, column=1)
    
    #set default value
    mp_scale.set(max_tokens)
    
    mp_lb = tk.Label(window, text="""Enquanto mais alto maior vai ser a sugestão. Sugiro não aumentar além do valor padrão (256).
                     é bom diminuir o valor quando você quer frz uma pergunta pra ele, converter um filme pra emoji,
                     traduzir/corrigir textos, etc.""", font=("Arial", 12), fg="black", width=73, height=5)
    mp_lb.grid(row=3, column=2)
     
    
    
    #presence_penalty
    pp_lb = tk.Label(window, text="Desencentivar Repetições", font=("Arial", 12), fg="black")
    pp_lb.grid(row=5, column=0)
    
    p = tk.DoubleVar()
    pp_scale = tk.Scale(window, from_= 0, to= 2, orient=tk.HORIZONTAL, sliderlength=60,  length=500, showvalue=1.0, tickinterval=0.0, resolution=0.1, variable=p)
    pp_scale.grid(row=5, column=1)
    ...
    pp_lb2.grid(row=5, column=2)
    
    
    
    #frequency_penalty
    fp_lb = tk.Label(window, text="Desencentivar Frequência", font=("Arial", 12), fg="black")
    fp_lb.grid(row=6, column=0)
    
    fp = tk.DoubleVar()
    fp_scale = tk.Scale(window, from_= 0, to= 2, orient=tk.HORIZONTAL, sliderlength=60,  length=500, showvalue=1.0, tickinterval=0.0, resolution=0.1, variable=fp)
    fp_scale.grid(row=6, column=1)
    #set default value
    fp_scale.set(frequency_penalty)
    
    fp_lb2 = tk.Label(window, text="""Enquanto mais baixo maior a chance dele ficar focado no mesmo assunto
                      (é basicamento o contrario do "Desencentivar Repetições") use numeros
                      altos quando você quer que ele responda uma pergunga, corrija um texto ou
                       converta um filme pra emoji""" , font=("Arial", 12), fg="black", width=73, height=5)
    fp_lb2.grid(row=6, column=2)
    ...
    def save_config():
        ...
        temperature = tp_scale.get()
        max_tokens = int(mp_scale.get())
        frequency_penalty = fp_scale.get()
        Presence_penalty = pp_scale.get()
        
        print(f'{temperature} {max_tokens} {frequency_penalty} {Presence_penalty}')
    #save button
    save_btn = tk.Button(window, text="Salvar", command=save_config, font=("Arial", 15), fg="black")
    ...
    def default_config():
        df_temperature = 0.7
        df_max_tokens = 256
        df_Presence_penalty = 0.0
        df_frequency_penalty = 0.0
        
        tp_scale.set(df_temperature)
        mp_scale.set(df_max_tokens)
        pp_scale.set(df_Presence_penalty) 
        fp_scale.set(df_frequency_penalty)
        
        save_config()

    #default button
    default_btn = tk.Button(window, text="Voltar para as configurações padroes", command=default_config, font=("Arial", 15), fg="black")
    default_btn.grid(row=9, column=2)   
...
def simple_sugestion(event=None):
    ...
    global max_tokens #enquanto maior o numero mais texto vai ser sugerido
    global frequency_penalty # enquanto mais alto mais pe no chão é sugestão 
    global frequency_penalty # enquanto mais alto mais mundo da lua
    
    print(temperature, max_tokens, frequency_penalty, frequency_penalty)

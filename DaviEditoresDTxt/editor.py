...
sujestion_icon = tk.PhotoImage(file="icons2/suggestion.png")
...
main_menu.add_cascade(label="Sugestão", menu=sujestion_icon)
...
def sugegstion(event=None, text='Quem é Bolsonaro?'):
    ...
    copy_button = tk.Button(window, text="Copy", command=lambda: copy(Sugestion.choices[0].text))
    copy_button.pack()
    label = tk.Label(window, text=Sugestion.choices[0].text)
    label.pack()
    window.mainloop()
...
view.add_checkbutton(label="Feramentas",onvalue=True,offvalue=False,  variable = show_toolbar, image=tool_bar_icon, compound=tk.LEFT, command= hide_toolbar)
